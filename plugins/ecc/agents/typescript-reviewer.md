...
以下为 TypeScript/JavaScript 特有的安全问题。通用的安全审查（XSS、SQL 注入、密钥泄露等）由 `security-reviewer` agent 负责。
...
> 全面的 OWASP Top 10 安全审查请使用 `security-reviewer` agent。
...
使用 `coding-standards` 加上 `frontend-patterns` 或 `backend-patterns` 获取 TypeScript/JavaScript 编码模式。通用安全检查由 `security-reviewer` agent 负责。
